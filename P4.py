...
# ---
# 
# * Estudiante: **Ricardo Corrales Vásquez**
# * Carné: **B72403**
# * Grupo: **2**
# 
# ---
...
# 4.1 Simulación de sistema de comunicaciones.
...
# Ya que se obtuvieron las imágenes respectivas, ahora es necesario obtener la forma de onda.



# Visualizar el cambio entre las señales
fig, ( ax2, ax3, ax4, ax5) = plt.subplots(nrows=4, sharex=True, figsize=(14, 7))


# La señal modulada por BPSK
ax2.plot(moduladora_S[0:600], color='b', lw=2) 
ax2.set_ylabel('$b(t)$')
# La señal modulada al dejar el canal
ax3.plot(senal_Tx[0:600], color='m', lw=2) 
ax3.set_ylabel('$s(t)$')
# La señal demodulada
ax4.plot(senal_Rx[0:600], color='r', lw=2) 
ax4.set_ylabel('$s(t) + n(t)$')
ax5.plot(senal_demodulada_C[0:600], color='g', lw=2) 
...
# 4.2 Determinar la densidad espectral de potencia para Tx

# Como recomendación se brindó usar el código trabajado en Lab4.
# Tiempo:
...
plt.plot(t, P, lw=6, color='y', label="Realizaciones (promedio)")



# El valor esperado:
E = np.mean(senal_Tx)*t 
plt.plot(t, E, '--' ,lw=2,color='m',label="Teórico" )




# Ahora se procede a realizar la gráfica correspondiente

# Información para la gráfica
plt.title("Proceso aletorio")
plt.xlabel("$t$")
plt.ylabel("$s(t)$")
plt.legend()
plt.show()
...
# 4.3 Prueba de estacionaridad y ergodicidad a Tx
...
plt.plot(f, 2.0/Nm * np.power(np.abs(senal_f[0:Nm//2]), 2), color='m')
plt.xlim(0, 20000)
plt.grid()
...
# Se tiene que sí existe ergocidad, de las tablas anteriores podemos
# observar que tanto para el valor teórico que para el valor de realizaciones son  muy parecidos o casi iguales
# por ende sí hay ergocidad.
